...
`public void interrupt()`：中断此线程（该方法的Thread实例所代表的线程），实际上只是给线程设置一个中断标志，线程仍会继续运行。

`public static boolean interrupted()`：判断当前线程（main线程，尽管调用者是thread）是否被中断（检查中断标志），返回并清除中断状态。二次调用返回false（状态已清除）。

`public boolean isInterrupted()`：判断线程是否被中断，不清除中断状态。

中断的线程会发生上下文切换，操作系统会保存线程信息，抢占到 CPU 后会从中断的地方接着运行（中断不是停止）

* sleep、wait、join 方法都会让线程进入阻塞状态，中断被阻塞的线程会抛出中断异常, 中断状态会被清空置为false。
  ...
  java.lang.InterruptedException: sleep interrupted
  	...
  	at Test.Test.lambda$main$0(Test.java:7)
  	at java.base/java.lang.Thread.run(Thread.java:842)
   打断状态: {}false
  ```

* 中断正常运行的线程：不会清空打断状态（true）
...
如果一个线程已经被 `park()` 阻塞，再调用 `interrupt()` 方法会取消线程的阻塞状态，并不会抛出 `InterruptedException` 异常。此时，`park()` 方法会立即返回，线程继续执行，但是中断状态不会被清除。
...
如果线程处于 `park()` 状态，它会检查中断标志位，如果中断标志位为 `true`，则会立即从 `park()` 方法返回，否则继续阻塞。
...
public class Main {
    public static void main(String[] args) throws InterruptedException {
        Thread t1 = new Thread(() -> {
            ...
            LockSupport.park();
            log.debug("unpark...");
            log.debug("打断状态:{}", Thread.currentThread().isInterrupted());   // isInterrupted()状态为true 下一行park不会阻塞
            LockSupport.park();	// 无法阻塞
            log.debug("unpark...");
        }, "t1");
        t1.start();
...
* `public final void suspend()`：**挂起（暂停）线程运行**
  ...
  废弃原因：若目标线程暂停时对系统资源持有锁，则目标线程恢复之前没有线程可以访问该资源，如果**恢复目标线程的线程**在调用 resume 之前会尝试访问此共享资源，则会导致死锁
...
public class Main {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        t1.join();
        t2.join();
        log.debug("{}", counter);	// 多线程环境下，counter结果可能不是0，因为++ --都不是原子操作，与执行顺序有关
    }
}
```
...
Hashtable table = new Hashtable();
// 线程1，线程2	
if( table.get("key") == null) {		// 线程1和线程2可能同时进入if
	table.put("key", value);
}
```
...
public class MyAspect {
    // 不安全
    private long start = 0L;	//可以做成局部变量解决
    @Before("execution(* *(..))")
...
**<font color="red">线程八锁</font>**

线程八锁就是考察 synchronized 锁住的是哪个对象，直接百度搜索相关的实例

1. <font color="blue">普通同步方法普通同步方法</font>：两个线程分别访问同一个对象的两个普通同步方法。由于普通方法默认使用的**锁对象是this**，因此这两个方法会互斥执行。
2. <font color="blue">静态同步方法和静态同步方法</font>：两个线程分别访问同一个类的两个静态同步方法。由于静态方法默认使用的**锁对象是类的Class对象**，因此这两个方法会互斥执行。
3. <font color="blue">静态同步方法和普通同步方法</font>：一个线程访问对象的普通同步方法，另一个线程访问类的静态同步方法。**一个是对象锁，一个是类锁，它们之间不会互斥执行。**
4. <font color="blue">两个对象</font>：两个线程分别访问两个不同对象的同步方法。**不同的锁对象，不会互斥执行**。
5. <font color="blue">普通同步方法和普通代码块</font>：同一个对象的普通同步方法和普通代码块之间是互斥的，因为它们使用的是**同一个对象实例的锁**。
6. <font color="blue">静态同步方法和静态代码块</font>：同一个类的静态同步方法和静态代码块之间是互斥的，因为它们使用的是**类的Class对象的锁**。
7. <font color="blue">静态方法和静态方法（同一类）</font>：两个线程分别访问同一个类的两个静态同步方法。由于静态方法默认使用的锁对象是**类的Class对象锁**，因此这两个方法会互斥执行。
8. <font color="blue">普通同步方法和静态代码块</font>：由于普通同步方法使用的是**对象实例的锁**（使用 synchronized 关键字修饰），而静态同步代码块使用的是类的 **Class 对象的锁**，因此它们使用的是不同的锁对象 。所以普通同步方法和静态同步代码块之间不会互斥执行，可以同时被多个线程访问。
